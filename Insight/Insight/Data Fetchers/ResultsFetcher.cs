...
namespace Insight.Data_Fetchers
{
    ...
    {
        public List<Fixture> GetResults(string league)
        {
            // get page source
            var source = new WebSourceFetcher().GetPageSource($"http://www.skysports.com/football/competitions/{league}/results");
        ...
        private List<Fixture> ExtractFixtures(string source, string league)
        {
            ...
            while (startPos > -1)
            {
                ...
                startPos = cut1.IndexOf("href");
                cut1 = cut1.Substring(startPos + 6);

                startPos = cut1.IndexOf("\"");
                string link = cut1.Substring(0, startPos);
                ...
                var homeScoreStr = scoreWorking.Substring(0, endPos2).Trim();

                // score2
                startPos = scoreWorking.IndexOf("matches__teamscores-side");
                scoreWorking = scoreWorking.Substring(startPos + 26);
                var endPos3 = scoreWorking.IndexOf("</");
                var awayScoreStr = scoreWorking.Substring(0, endPos3).Trim();
                ...
                try
                {
                    hScore = int.Parse(homeScoreStr);
                    aScore = int.Parse(awayScoreStr);
                }
                catch (Exception) { }

                Fixture newFixture = new Fixture(homeTeam, awayTeam, league, date + " @ " + kickoff, link, hScore, aScore);

                newFixture.AnalyseReport();
